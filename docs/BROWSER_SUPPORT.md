...
| Shopify development-store page | Verified through the Codex in-app WebMCP client | Password-protected top-level Online Store page registered and executed the exact six CoDesign tools alongside Shopify's native catalog/cart tools; real artwork, two previews, production validation, mobile review controls, atomic rejection and zero-write Revert passed | Complete for this development-store release; do not relabel as consumer ChatGPT web or current native-Chrome evidence |
