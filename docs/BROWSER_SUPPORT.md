...
| Codex desktop in-app browser | Verified on current rebranded tote | Exact-six discovery, real artwork, multiple coherent passes, two distinct 640 x 640 renderer previews, production validation, atomic invalid-value rejection and zero-save Revert | Complete for the current tote release; do not relabel as consumer ChatGPT web |
| Shopify development-store page | Verified as a hosted human configurator and code-identity proof | Password-protected top-level Online Store page retained Shopify's native WebMCP and standard-action scripts, loaded the same tested CoDesign application digest from the Shopify CDN, passed desktop/mobile interaction and preserved a rollback theme | Do not claim an exact-six Shopify-page agent invocation until a supported client exposes the page tools on this password-protected store |
