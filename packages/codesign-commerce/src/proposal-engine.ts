...
export class ProposalEngine<Snapshot = unknown> {
  readonly manifest: ConfiguratorManifest;
  readonly adapter: GuardedWorkspaceAdapter<Snapshot>;
  ...
  constructor(manifest: ConfiguratorManifest, rawAdapter: WorkspaceAdapter<Snapshot>) {
  ...
  async apply(rawInput: unknown, execution: ProposalEngineExecutionOptions = {}): Promise<ProposalEngineSuccessResult | ProposalEngineErrorResult> {
    ...
    try {
      ...
      const validation = await this.adapter.validateWorkspace(reduced.state);
      ...
      await this.adapter.previewWorkspace(reduced.state);
      this.#checkBoundary(execution.signal, this.#active.baseRevision);
  ...
  async revert(): Promise<{ reverted: true; persisted: false } | ProposalEngineErrorResult> {
    ...
    if (this.#status !== "reviewable") return errorResult("OPERATION_IN_PROGRESS", "The proposal is not ready to revert", true);
  ...
  async destroy(): Promise<void> {
    ...
    if (!["commit-retry", "commit-uncertain", "committing"].includes(this.#status)) {
      try { await this.adapter.restoreSnapshot(this.#active.snapshot); } catch { /* teardown cannot save */ }
    }
    try { await this.adapter.endProposalMode("teardown"); } catch { /* page teardown continues */ }
    this.#clear();
  }
  ...
  async #restoreReviewablePreview(): Promise<boolean> {
    ...
    try {
      await this.adapter.previewWorkspace(this.#active.state);
      this.#setStatus("reviewable");
      return true;
    } catch {
      this.#setStatus("stale");
