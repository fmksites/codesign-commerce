...
| Deterministic QA | `PASS` | Fresh clone of corrected public commit `e137a3b` passes 95 tests, strict typecheck/build, exact bundle verification, boundary/docs/judge/eval checks, `git diff --check`, and empty status. The current private snapshot passes syntax, focused ESLint, 41 files/199 tests, typecheck, production build, and the complete 142-case desktop/mobile suite with 141 passes and 1 expected skip. | Current public and private local candidates have complete deterministic evidence. The current private suite includes persistent temporary-status wording after both proposed-colourway switches and unchanged zero-write invariants. Production activation and live-Shopify verification remain separate gates. |
...
| Actual-browser verification | `IN_PROGRESS` | The sole tote demo exposes exactly five tools, stages and validates two 50-unit variants with `persisted: false`, and Reverts exactly. On tagged QA `codesign-user-qa`, Codex's real in-app WebMCP browser completed both the scripted five-tool check and a separate natural-language rehearsal. The latter recovered from a correctly rejected inconsistent 60/120 intermediate, staged and validated 2 × 60, switched Cream → Rose, preserved `Temporary proposal not saved`, and re-read the unchanged committed one-design/20-pair state with no browser errors or warnings. | Zero-traffic supported agent-browser evidence passes in `docs/evidence/KORRHAUS_READ_ONLY_COLOURWAY_QA.md`. The proposal is staged for owner hands-on QA. The separate ChatGPT Desktop shell, permission and recent-tool-activity check remains pending. After a new fixtures-off candidate and separately approved activation, verify the same bytes on the real Shopify route; no live-Shopify WebMCP claim exists yet. |
| Public deployment | `IN_PROGRESS` | Corrected topology commit `4850606` and hosted CI `33062478946` pass; current read-only QA evidence commit `4592aea` is public and exact hosted CI run `33079821637` passes the full gate set. See `docs/evidence/PUBLIC_REPOSITORY_RELEASE.md`. | Public source and current QA evidence publication are complete. Judge-site hosting remains separately approval-gated after the real KORRHAUS shop is live-verified; tote is the only standalone demo. |
| KORRHAUS no-traffic proof | `PASS` | `docs/evidence/KORRHAUS_READ_ONLY_COLOURWAY_QA.md`; Cloud Build `2985b639-7524-4a09-85ac-f367b78865e7`; immutable `181917…` image; healthy tagged `codesign-review-qa2`; exact deployed asset hash; fresh-origin five-tool and read-only live-proof check; zero revision errors; `0%` ordinary traffic | Live traffic remains `100%` on feature-off `sock-logo-v2`. Service-template defaults are fixtures-off and WebMCP-off through retired hold revision `codesign-review-hold2`. Historical `codesign-prod1` and now-superseded `codesign-prod2` must never be promoted. |
| Production promotion | `WAITING_FOR_APPROVAL` | Current owner-test surface `codesign-user-qa` is ready at `0%`; exact image is `sha256:181917…d9e3c`; rollback baseline is `korrhaus-admin-app-sock-logo-v2` at `100%`; service-template default is WebMCP-off | First complete owner hands-on QA. Then deploy the exact image as a new fixtures-off zero-traffic candidate and verify it. Only after that separate evidence may Codex ask for explicit production-traffic approval. The real Shopify route must then be verified in an actual supported WebMCP browser; roll back immediately on a critical failure. |
...
| 27 Aug 2026 | Add read-only inspection of proposed colourways before owner QA. | User | While a proposal is awaiting human review, its existing colourway tabs may switch the same live preview. All edit, upload, autosave and persistence paths remain locked; `Temporary proposal not saved` must remain visible. |
| 27 Aug 2026 | Test the UX on a tagged QA surface before any production candidate or traffic change. | User | Production stays unchanged. The corrected exact image must pass local regression, fresh-origin in-app WebMCP verification and owner hands-on QA before a new fixtures-off candidate is created. |
