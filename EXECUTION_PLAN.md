...
| Actual-browser verification | `IN_PROGRESS` | The sole tote demo exposes exactly five tools, stages and validates two 50-unit variants with `persisted: false`, and Reverts exactly. On fresh-origin tagged QA `codesign-user-qa`, the real in-app WebMCP browser discovered all five tools, read one design/20 pairs, listed eight option groups, staged and validated 2 × 60, switched the live proof Rose → Cream → Rose, preserved `Temporary proposal not saved`, kept all mutation/upload controls locked, and re-read the unchanged committed state with no browser errors or warnings. | Zero-traffic actual-browser evidence passes in `docs/evidence/KORRHAUS_READ_ONLY_COLOURWAY_QA.md`. The proposal is staged for owner hands-on QA. After a new fixtures-off candidate and separately approved activation, verify the same bytes on the real Shopify route; no live-Shopify WebMCP claim exists yet. |
