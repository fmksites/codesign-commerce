...
| Public foundation | `IN_PROGRESS` | Clean clone of evidence commit `f114581` passed `npm ci`, 45 tests, strict source/test typecheck, module/browser/reference build, public-boundary over 66 tracked candidates, docs, eval-corpus, browser-bundle digest `sha256:78ece1955a7416878c50a7f01325c702aa609974fb0cf816b1be3048e7f9819a`, and an empty final status; Apache-2.0 is present | Local foundation is reproducible. Hosted CI and the public remote repository remain external gates. |
...
| Complete tool suite | `IN_PROGRESS` | Public source commit `ea54e71` registers exactly five tools; `docs/evidence/NORTH_FORM_FIVE_TOOL.md` records actual discovery, option listing, proposal extension, design creation, validation, and Revert | Public behavior passes. The private flagship adapter still needs the five-tool bundle and second-design seam before this milestone closes. |
| Public reference | `IN_PROGRESS` | The public KORRHAUS reference now begins with one 120-pair design and completes the exact two-colourway scenario through five actual webpage calls; desktop/mobile evidence in `docs/evidence/NORTH_FORM_FIVE_TOOL.md` | Current evidence commit still needs a clean-clone run; final judge reset/instructions remain Phase 5 work. |
| Tote portability proof | `NOT_STARTED` | — | Conditional. |
| Deterministic QA | `IN_PROGRESS` | 54 tests pass across manifest, transaction, clone, validation, review, adapter, and WebMCP contracts | Tote, expanded safety cases, all-adapter matrix, and final clean clone remain. |
| WebMCP evals | `NOT_STARTED` | — | — |
| Actual-browser verification | `IN_PROGRESS` | Public five-tool North Form scenario and Revert pass in the actual WebMCP-capable in-app browser; corrected desktop/mobile review states inspected | Private five-tool, tote, negative prompt, final immutable-build, and deployed checks remain. |
