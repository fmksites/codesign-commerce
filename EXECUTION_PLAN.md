...
| Public foundation | `IN_PROGRESS` | Clean clone of evidence commit `37682a2` passed `npm ci`, 54 tests, strict source/test typecheck, module/browser/reference build, public-boundary over 69 tracked candidates, docs, eval-corpus, browser-bundle digest `sha256:3723b4937086323c1536406f2072efbd54da702ec63d7c2f94d32ea768f101f6`, and an empty final status; Apache-2.0 is present | Complete public five-tool source is locally reproducible. Hosted CI and the public remote repository remain external gates. |
...
| Public reference | `IN_PROGRESS` | The public KORRHAUS reference completes the exact two-colourway scenario through five actual webpage calls; desktop/mobile evidence in `docs/evidence/NORTH_FORM_FIVE_TOOL.md`; clean clone of evidence commit `37682a2` passed all local gates | Five-tool source and evidence are reproducible. Final anonymous reset and judge instructions remain Phase 5 work. |
| Tote portability proof | `NOT_STARTED` | — | Conditional. |
| Deterministic QA | `IN_PROGRESS` | 54 public tests pass across manifest, transaction, clone, validation, review, adapter, and WebMCP contracts; private typecheck/build, 12 focused page tests, and 95-pass/1-skip full Designer E2E pass | Tote, expanded safety cases, all-adapter matrix, and final clean clone remain. |
| WebMCP evals | `NOT_STARTED` | — | — |
| Actual-browser verification | `IN_PROGRESS` | Public and private five-tool North Form scenarios and Revert pass in the actual WebMCP-capable in-app browser; pre-proposal visibility, committed-state isolation, and corrected responsive review states inspected | Tote, negative prompt, final immutable-build, and deployed checks remain. |
